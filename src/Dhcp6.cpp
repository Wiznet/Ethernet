...
int Dhcp6Class::beginWithDHCPV6(uint8_t *mac, unsigned long timeout, unsigned long responseTimeout)
{
	_dhcpLeaseTime = 0;
	_dhcpT1 = 0;
	_dhcpT2 = 0;
...
void Dhcp6Class::reset_DHCPV6_lease()
{
	memset(_dhcpGua, 0, 16 * 5);
}

//return:0 on error, 1 if request is sent and response is received
int Dhcp6Class::request_DHCPV6_lease()
{
...
void Dhcp6Class::InitDhcpV6Option(unsigned asize, unsigned agrowby)
{
	size = asize;
	growby = agrowby;
	num = 0;
...
void Dhcp6Class::DumpDhcpV6Option(char *sMark)
{
	unsigned i;
...
void Dhcp6Class::AppendDhcpV6Option(uint8_t value)
{
	uint32_t need;
...
uint8_t Dhcp6Class::send_DHCPV6_INFOREQ(void)
{
	uint16_t j;
	uint8_t ip[16];
	uint8_t rip_msg_size;
...
void Dhcp6Class::presend_DHCPV6()
{
}

int Dhcp6Class::checkLease()
{
...
IP6Address Dhcp6Class::getGua()
{
	return IP6Address(_dhcpGua);
}
#if 0
IP6Address Dhcp6Class::getSubnetMask()
{
	return IP6Address(_dhcpSubnetMask);
}

IP6Address Dhcp6Class::getGatewayIp()
{
	return IP6Address(_dhcpGatewayIp);
}

IP6Address Dhcp6Class::getDhcpServerIp()
{
	return IP6Address(_dhcpDhcpServerIp);
}

IP6Address Dhcp6Class::getDnsServerIp()
{
	return IP6Address(_dhcpDnsServerIp);
}
#endif

void Dhcp6Class::printByte(char *buf, uint8_t n)
{
	char *str = &buf[1];
	buf[0] = '0';
	do
